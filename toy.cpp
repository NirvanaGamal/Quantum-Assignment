...
static void toy_fill_small(short* buf, int n)
{
    ...
    for (int k = 0; k < n; ++k)
    {
        short val = rand(); //we can write that val = rand() without 3 or 0xF 
...
{
    dst[0] = ((dst[0] & -add_to_dst) + a[0] * b[0] + NEG(a[3]) * b[1] + NEG(a[2]) * b[2] + NEG(a[1]) * b[3]) % TK_Q;
    dst[1] = ((dst[1] & -add_to_dst) + a[1] * b[0] + a[0] * b[1] + NEG(a[3]) * b[2] + NEG(a[2]) * b[3]) % TK_Q;
    dst[2] = ((dst[2] & -add_to_dst) + a[2] * b[0] + a[1] * b[1] + a[0] * b[2] + NEG(a[3]) * b[3]) % TK_Q;
    dst[3] = ((dst[3] & -add_to_dst) + a[3] * b[0] + a[2] * b[1] + a[1] * b[2] + a[0] * b[3]) % TK_Q;
...
void toy_test()
{
    short A[TK_K * TK_K * TK_N], s[TK_K * TK_N], e[TK_K * TK_N], r[TK_K * TK_N], e1[TK_K * TK_N], e2[TK_N];
    for (int k = 0; k < TK_K * TK_K * TK_N; ++k)
        A[k] = rand() % TK_Q;
    toy_fill_small(s, TK_K * TK_N);
    toy_fill_small(e, TK_K * TK_N);
